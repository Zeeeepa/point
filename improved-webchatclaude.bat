...
set "LOGFILE=%~dp0logs\webchatclaude_%LOGDATE%.log"
...
REM Start logging
echo Starting Web Claude OpenAI API endpoint at %TIME% > "%LOGFILE%"
...
if not exist "config\config.json" (
    ...
    echo NOTE: You will need to log in to Claude when the browser opens. | %TEE%
)
...
echo Building and starting the Web Claude OpenAI API compatible endpoint... | %TEE%
echo Running: go run web-integration-agent.go --mode claude --port 8081 | %TEE%
go run web-integration-agent.go --mode claude --port 8081
set EXIT_CODE=%ERRORLEVEL%
echo Web Claude OpenAI API endpoint exited with code %EXIT_CODE% | %TEE%

REM If the program exits, pause to see any error messages
echo. | %TEE%
echo The Web Claude OpenAI API endpoint has stopped. | %TEE%
echo Press any key to exit... | %TEE%
pause > nul
