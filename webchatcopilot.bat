...
powershell -Command "& {if (Test-NetConnection -ComputerName localhost -Port 8080 -InformationLevel Quiet -WarningAction SilentlyContinue -ErrorAction SilentlyContinue) { Write-Host 'Warning: Port 8080 is already in use. The service may fail to start.' }}"
...
echo.
echo Note: This will create an OpenAI API compatible endpoint at http://localhost:8080
echo You can use this endpoint with any OpenAI API client by setting the base URL to http://localhost:8080
echo.
