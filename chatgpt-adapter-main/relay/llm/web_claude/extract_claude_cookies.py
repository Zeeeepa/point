#!/usr/bin/env python3
"""
Claude Cookie Extractor
Extract cookies from browser for Claude.ai authentication.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Extract Claude.ai cookies from Firefox browser')
    parser.add_argument('-o', '--output', default='claude_cookies.json', help='Output file path')
    ...
    try:
        # Extract cookies for Claude.ai
        cookies = extract_cookies(temp_db, "claude.ai")
        
        if not cookies:
            print("No cookies found for Claude.ai.")
            return
        
        # Save cookies to JSON file
        save_as_json(cookies, args.output)
        
        print("\nTo use these cookies with the web_claude adapter, include them in your API request:")
